...
function AppInner() {
  ...
  const [discordServerName, setDiscordServerName] = useState("Rampage");
    ...
    try {
      ...
      if (data) {
        data.forEach(row=>{
          ...
          if (row.key === "discord_invite_link" && row.value) setDiscordInviteLink(row.value);
          if (row.key === "discord_server_name" && row.value) setDiscordServerName(row.value);
          ...
          {activeNav==="settings"&&(
            <div className="page" style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:18,maxWidth:900}}>
              ...
              <div style={{background:"rgba(88,101,242,0.06)",border:"1px solid rgba(88,101,242,0.22)",borderRadius:18,padding:"22px"}}>
                <h3 style={{fontFamily:"'Rajdhani',sans-serif",fontSize:17,fontWeight:700,color:"#a5b4fc",marginBottom:6,letterSpacing:"0.04em"}}>🎮 Discord Integration</h3>
                <p style={{color:"#3d5070",fontSize:11.5,marginBottom:16,lineHeight:1.6}}>Connect a Discord webhook for auto-notifications. Set an invite link so members can join your Discord server directly from the app.</p>
                ...
                {isAdmin&&(
                  <div style={{marginBottom:13}}>
                    ...
                    <input className="dark-input" placeholder="https://discord.com/api/webhooks/..." value={discordWebhook} onChange={e=>setDiscordWebhook(e.target.value)} />
                  </div>
                )}
                {isAdmin&&<button className="btn" onClick={()=>{setDiscordConnected(true);showToast("🎮 Discord connected!");}}
                  style={{background:discordConnected?"rgba(52,211,153,0.15)":"linear-gradient(135deg,#5865f2,#7289da)",border:discordConnected?"1px solid rgba(52,211,153,0.3)":"none",color:discordConnected?"#34d399":"#fff",padding:"10px 22px",fontSize:13,marginBottom:18}}>
                  {discordConnected?"✅ Connected":"Connect Webhook"}
                </button>}

                <div style={{borderTop:"1px solid rgba(88,101,242,0.18)",paddingTop:16,marginTop:isAdmin?4:0}}>
                  <label style={{display:"block",color:"#3d5070",fontSize:10.5,fontWeight:700,marginBottom:10,textTransform:"uppercase",letterSpacing:"0.08em"}}>Discord Server Widget <span style={{color:"#7289da",fontWeight:400,textTransform:"none",fontSize:10}}>(visible to all members)</span></label>

                  {/* Admin: edit fields */}
                  {isAdmin&&(
                    <div style={{display:"flex",flexDirection:"column",gap:10,marginBottom:14}}>
                      <div>
                        <label style={{display:"block",color:"#3d5070",fontSize:10,fontWeight:700,marginBottom:5,textTransform:"uppercase",letterSpacing:"0.07em"}}>Server Name <span style={{color:"#64748b",fontWeight:400,textTransform:"none",fontSize:10}}>(shown beside logo — max 192px)</span></label>
                        <input className="dark-input" placeholder="e.g. Rampage" value={discordServerName} onChange={e=>setDiscordServerName(e.target.value)} style={{fontSize:13}} />
                      </div>
                      <div>
                        <label style={{display:"block",color:"#3d5070",fontSize:10,fontWeight:700,marginBottom:5,textTransform:"uppercase",letterSpacing:"0.07em"}}>Invite Link</label>
                        <input className="dark-input" placeholder="https://discord.gg/yourcode" value={discordInviteLink} onChange={e=>setDiscordInviteLink(e.target.value)} style={{fontSize:13}} />
                      </div>
                      <button className="btn" onClick={async()=>{
                        await supabase.from("settings").upsert({key:"discord_invite_link",value:discordInviteLink},{onConflict:"key"});
                        await supabase.from("settings").upsert({key:"discord_server_name",value:discordServerName},{onConflict:"key"});
                        showToast("✅ Discord widget saved — visible to all members!");
                      }} style={{background:"linear-gradient(135deg,#5865f2,#7289da)",color:"#fff",padding:"10px 20px",fontSize:13,alignSelf:"flex-start"}}>
                        💾 Save Widget
                      </button>
                    </div>
                  )}

                  {/* Widget preview / live widget for all users */}
                  {discordInviteLink ? (
                    <a href={discordInviteLink} target="_blank" rel="noopener noreferrer"
                      style={{display:"inline-flex",alignItems:"center",gap:14,background:"linear-gradient(135deg,rgba(88,101,242,0.18),rgba(114,137,218,0.1))",border:"1px solid rgba(88,101,242,0.4)",borderRadius:16,padding:"14px 20px",cursor:"pointer",textDecoration:"none",transition:"all 0.2s",maxWidth:320,boxShadow:"0 4px 20px rgba(88,101,242,0.15)"}}
                      onMouseEnter={e=>{e.currentTarget.style.background="linear-gradient(135deg,rgba(88,101,242,0.28),rgba(114,137,218,0.18))";e.currentTarget.style.transform="translateY(-2px)";e.currentTarget.style.boxShadow="0 8px 28px rgba(88,101,242,0.3)";}}
                      onMouseLeave={e=>{e.currentTarget.style.background="linear-gradient(135deg,rgba(88,101,242,0.18),rgba(114,137,218,0.1))";e.currentTarget.style.transform="translateY(0)";e.currentTarget.style.boxShadow="0 4px 20px rgba(88,101,242,0.15)";}}>
                      {/* Discord logo SVG */}
                      <div style={{width:Math.min(logoSize||76,192),height:Math.min(logoSize||76,192),maxWidth:56,maxHeight:56,flexShrink:0,background:"#5865f2",borderRadius:14,display:"flex",alignItems:"center",justifyContent:"center",boxShadow:"0 4px 14px rgba(88,101,242,0.45)"}}>
                      ...
                      <div>
                        <div style={{fontFamily:"'Rajdhani',sans-serif",fontSize:17,fontWeight:700,color:"#e2e8f0",letterSpacing:"0.04em",maxWidth:192,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{discordServerName||"Discord Server"}</div>
                        <div style={{fontSize:10.5,color:"#7289da",fontWeight:600,marginTop:2,letterSpacing:"0.05em"}}>Click to join ↗</div>
                      </div>
                    </a>
                  ) : (
                    <div style={{background:"rgba(255,255,255,0.03)",border:"1px dashed rgba(88,101,242,0.3)",borderRadius:14,padding:"18px 20px",color:"#3d5070",fontSize:12,textAlign:"center"}}>
                      {isAdmin ? "⬆️ Fill in the server name and invite link above, then save." : "No Discord server linked yet — ask your admin to add one."}
                    </div>
                  )}
                </div>
              </div>
